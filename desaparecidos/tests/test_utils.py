...
class TestUtils(TestCase):
    def test_paginate_data(self):
        data = {
            'key 1': dict(zip(range(10), range(20))),
            'key 2': dict(zip(range(10), range(20, 40)))
        }

        page_data = paginate(data, page=1, page_size=10)

        expected_data = {
            'key 1': dict(zip(range(10), range(10))),
            'key 2': dict(zip(range(10), range(20, 30)))
        }
        self.assertEqual(page_data, expected_data)

    def test_paginate_data_2(self):
        data = {
            'key 1': dict(zip(range(100), range(100))),
            'key 2': dict(zip(range(100), range(100, 200)))
        }

        page_data = paginate(data, page=2, page_size=10)

        expected_data = {
            'key 1': dict(zip(range(10), range(10, 20))),
            'key 2': dict(zip(range(10), range(110, 120)))
        }
        self.assertEqual(page_data, expected_data)
    ...
    def test_paginate_data_page_size(self):
        ...
        expected_data = {
            'key 1': dict(zip(range(20), range(40, 60))),
