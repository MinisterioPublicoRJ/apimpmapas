...
PESSOAS_POSSIVEIS AS (
    ...
    FROM FILTRADOS_ALVO
    LEFT JOIN
            SILD_ENDERECO_FATO ON FILTRADOS_ALVO.SNCA_DK = SILD_ENDERECO_FATO.SIES_SNCA_DK
    LEFT JOIN
            CORP.CORP_CIDADE
...
SELECT
    ...
    ALVO_DT_FATO,
    CASE
       WHEN VITM_CAND.VTMA_DT_NASCIMENTO IS NOT NULL
         THEN (ALVO_DT_FATO - VITM_CAND.VTMA_DT_NASCIMENTO) / 365.2425
       WHEN IDADE_APARENTE_CAND.SIDP_NM_IDADE_APARENTE IS NOT NULL
         THEN
    (CAST(NVL(SUBSTR(SIDP_NM_IDADE_APARENTE, 1, INSTR(SIDP_NM_IDADE_APARENTE, '-')-1), 1) AS INT) +
    CAST(REPLACE(REPLACE(SUBSTR(SIDP_NM_IDADE_APARENTE, INSTR(SIDP_NM_IDADE_APARENTE, '-')+1), '>',''), '<','') AS INT)) / 2 + (ALVO_DT_FATO - INFO_CAND.SNCA_DT_FATO) / 365.2425
...
FROM normalizada NORM
LEFT JOIN SILD_SINDICANCIA INFO_CAND
    ON NORM.SNCA_IDENTIFICADOR_TODOMUNDO = INFO_CAND.SNCA_IDENTIFICADOR_SINALID
LEFT JOIN
    SILD_ENDERECO_FATO END_CAND ON INFO_CAND.SNCA_DK = END_CAND.SIES_SNCA_DK
LEFT JOIN
...
ON VITM_CAND.VTMA_DK = INFO_CAND.SNCA_VTMA_DK
LEFT
    JOIN SILD.SILD_IDADE_APARENTE IDADE_APARENTE_CAND
...
LEFT JOIN SILD_COR_PELE COR_PELE_CAND
    ON VITM_CAND.VTMA_SCPE_DK = COR_PELE_CAND.SCPE_DK
LEFT JOIN SILD_ALTURA ALTURA_CAND
    ON VITM_CAND.VTMA_SATR_DK = ALTURA_CAND.SATR_DK
ORDER BY NORM.N_SEX_SCORE+NORM.N_DATE_SCORE+NORM.N_DIFERENCA_IDADE_APARENTE+NORM.N_DISTANCIA
"""
